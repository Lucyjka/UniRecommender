...
def generate_prompt(user_interests, df):
    grouped_programs = (
        df.groupby(["iscedName", "profileName","levelCode", "mainInstitutionKind"], group_keys=False)["mainInstitutionName"]
        .unique()
    ...
    program_data = "\n".join(
        [
            f"Kierunek: {row['iscedName']}\nProfil: {row['profileName']}\nRodzaj instytucji: {row['mainInstitutionKind']}\nStopień: {row['levelCode']}\nUczelnie: {', '.join(row['mainInstitutionName'])}"
    ...
    prompt = (
        ...
        f"Format odpowiedzi:\n"
        f"Według Twoich wytycznych najbardziej rekomendowanymi przeze mnie kierunkami studiów są:\n\n"
